...
vcl 4.0;
# Based on: https://github.com/mattiasgeniar/varnish-4.0-configuration-templates
...
sub vcl_recv {
  ...
  if (req.url ~ "marketer-pro-core.php" || req.url ~ "hullo-chat-core.php") {
	return (pass);
  }
  if (req.url ~ "load-scripts.php") {
	return (pass);
  }
  if (req.url ~ "^/(cart|my-account|checkout|addons|wishlist|order)" || req.url ~ "\?add-to-cart=" || req.url ~ "\?wc-ajax=") {
    return (pass);
  }
  if (req.http.Cookie ~ "wordpress_" || req.http.Cookie ~ "comment_" || req.http.Cookie ~ "woocommerce_items_in_cart") {
    return (pass);
  }
